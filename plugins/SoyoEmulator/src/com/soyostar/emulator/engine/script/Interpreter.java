package com.soyostar.emulator.engine.script;
...
public class Interpreter {
    ...
    private void doDialog(Command command) {
        se.addEvent(new Event(Event.DIALOG, script.row, script.col, command.param.split(" ")));
        curIndex++;

    }

    private void doSound(Command command) {
        se.addEvent(new Event(Event.SOUND, script.row, script.col, command.param.split(" ")));
        curIndex++;
    }

    private void doWait(Command command) {
        se.addEvent(new Event(Event.WAIT, script.row, script.col, command.param.split(" ")));
        curIndex++;
    }

    private void doItemShop(Command command) {
        se.addEvent(new Event(Event.ITEMSHOP, script.row, script.col, command.param.split(" ")));
        curIndex++;
    }

    private void doEquipShop(Command command) {
        se.addEvent(new Event(Event.EQUIPSHOP, script.row, script.col, command.param.split(" ")));
        curIndex++;
    }
    ...
    private void doMap(Command command) {
        se.addEvent(new Event(Event.MAP, script.row, script.col, command.param.split(" ")));
        curIndex++;
    }

    private void doFace(Command command) {
        se.addEvent(new Event(Event.FACE, script.row, script.col, command.param.split(" ")));
        curIndex++;
    }

    private void doMove(Command command) {
        se.addEvent(new Event(Event.MOVE, script.row, script.col, command.param.split(" ")));
        curIndex++;
    }

    private void doFight(Command command) {

        se.addEvent(new Event(Event.FIGHT, script.row, script.col, command.param.split(" ")));
        curIndex++;
    }
}
