...
public class TestController extends AbController {
	...
	public TestController() {
		...
		lTab.setTitleBackground(skin.createAlphaBg(lTab.getTitleWidth(),
				lTab.getTitleHeight(), false), skin.createBlueBg(
				lTab.getTitleWidth(), lTab.getTitleHeight(), false), skin
				.createAlphaBg(lTab.getTitleWidth(), lTab.getTitleHeight(),
						false));
		...
		String[] imags = { "001-Weapon01", "002-Weapon02", "003-Weapon03",
				"004-Weapon04", "005-Weapon05", "006-Weapon06", "007-Weapon07",
				"008-Weapon08", "009-Shield01", "010-Head01" };
		for (int i = 0; i < 10; i++) {
			ListItem li = new ListItem();
			li.addString("第" + i + "个选项", 40, 10, Painter.LT, Color.WHITE);
			li.addString("名称" + i, 150, 10, Painter.LT, Color.BLUE);
			li.addImage(new Image("res/image/icon/equip/" + imags[i] + ".png"),
					10, 40 / 2, Painter.LV);
			lList.addListItem(li);
		}
		lList.setSelectedIndex(8);
		bg.addWidget(lList);
		// Action action = new ShadeAction(bg, Color.BLACK, Color.RED, 20,
		// 1000);
		// action.activate();
		...
		//
		// lb = new
		// LButton(Image.createImage("res/image/battler/001-Fighter01.png"),
		// Image.createImage("res/image/battler/002-Fighter02.png"));
		// lb.setText("LButton");
