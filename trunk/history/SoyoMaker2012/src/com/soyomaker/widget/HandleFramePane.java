...
import com.soyomaker.util.TileTransferable;
...
public class HandleFramePane extends JPanel implements DropTargetListener, Runnable, Scrollable, MouseListener, MouseMotionListener, KeyListener {
     ...
     *
     */
    public static final int FRAME_PANEL_W = 480;
    /**
     *
     */
    public static final int FRAME_PANEL_H = 480;
            ...
            for (int i = 0, n = curFrame.getTiles().size(); i < n; i++) {
                ...
                if (clip.frameContains(cx, cy)) {
                    this.setToolTipText("<html>切块ID:" + data.getCurProject().getClips().indexOf(clip.getOriginal())
                            ...
                            + "<br>x:" + (clip.getFramePoint().x - FRAME_PANEL_W / 2)
                            + "<br>y:" + (clip.getFramePoint().y - FRAME_PANEL_H / 2)
                            + "<br>宽度:" + clip.getW()
                            + "<br>高度:" + clip.getH()
                            + "</html>");
                    return;
