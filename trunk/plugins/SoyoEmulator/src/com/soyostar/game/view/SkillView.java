...
import com.soyostar.game.control.SkillControl;
...
public class SkillView extends BaseView {
    ...
    public void paint(Painter painter) {
        ...
        painter.drawString("技 能", gd.screenWidth / 2, (th - painter.getClipHeight()) / 2, Const.Anchor.HT);
        /**
         * 列表
         */
        //底框
        Tools.drawDialog(painter, 0, th, gd.screenWidth, ch, Painter.DIALOG_LIGHT);
        //列表
        ...
        Tools.drawTable(painter, (gd.screenWidth - cellW) / 2 - gap / 2, th, cellW, cellH, gd.skill_showNum, space, skills, Color.BLACK, gd.skill_topIndex, gd.skill_curIndex, Const.Anchor.HV, Painter.NODIALOG, Painter.CELL_DEEP);
        ...
        if (gd.player.bag.get(Bag.SKILL, gd.skill_curIndex) != null) {
            Tools.drawWordWrapString(painter, gd.player.bag.get(Bag.SKILL, gd.skill_curIndex).intro, gap / 2, th + ch + gap / 2, gd.screenWidth - gap, ih - gap, Color.WHITE);

        }
        /**
         * 滚动条
         */
        Tools.drawScrollbar(painter, Painter.SCROLLBAR_VERTICAL, (gd.screenWidth - cellW) / 2 + cellW + gap / 2, th + space, th + ch - space, gd.skill_topIndex, gd.skill_showNum, gd.player.bag.getList(Bag.SKILL).length);
