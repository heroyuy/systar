...
public class LTab extends Widget {
    ...
    public void setTitleBackground(Image unSelected, Image selected, Image disabled) {
    ...
    public void paint(Painter painter) {
        ...
        for (int i = 0; i < tabList.size(); i++) {
            if (i == selectedIndex) {
                painter.drawImage(titleBackground[1], i * titleWidth, 0, Painter.LT);
            } else {
                painter.drawImage(titleBackground[0], i * titleWidth, 0, Painter.LT);
            }
            painter.drawString(tabList.get(i).title, i * titleWidth + titleWidth / 2, titleHeight / 2, Painter.HV);
        }
        //绘制内容区域
        ...
        if (contentWidget.isVisible()) {
            ...
            ((GraphicsPainter) painter).clipRect(0, 0, contentWidget.getWidth(), contentWidget.getHeight());
    ...
    public boolean dispatchTouchEvent(TouchEvent te) {
        ...
        if (tey <= titleHeight) {
            ...
            tabWidget.dispatchTouchEvent(new TouchEvent(te.getX() - tabWidget.getX(), te.getY() - titleHeight - tabWidget.getY(), te.getType()));
            state = true;
        }
        return state;
    }
    ...
    private void updateTitleBackground() {
        for (int i = 0; i < titleBackground.length; i++) {
            if (titleBackground[i] != null) {
                titleBackground[i] = Image.zoomImage(titleBackground[i], titleWidth, titleHeight);
            }
        }
    }
