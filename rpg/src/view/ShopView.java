...
public class ShopView extends BaseView {
    ...
    public void paint(Graphics g) {
        ...
        g.drawString(gd.shop_type == 0 ? "商店—物品" : "商店—装备", gd.screenWidth / 2, (th - g.getFont().getHeight()) / 2, Const.Anchor.HT);
        /**
         * 列表
         */
        //底框
        ...
        g.drawString("现有金钱：" + gd.player.money, 48 * 2 + 2 * gap, th + (24 - g.getFont().getHeight()) / 2, Graphics.TOP | Graphics.LEFT);
        //列表
        ...
        Painter.drawTable(g, (gd.screenWidth - cellW) / 2 - gap / 2, th + 3 * gap, cellW, cellH, gd.shop_showNum, space, names, Const.Color.BLACK, gd.shop_topIndex, gd.shop_curIndex, Const.Anchor.HV, Painter.NODIALOG, Painter.CELL_DEEP);
        ...
        //
        if (names.length > 0) {
            Painter.drawWordWrapString(g, gd.shop_tabIndex == 0 ? gd.shop_items_buy[gd.shop_curIndex].intro : gd.shop_items_sell[gd.shop_curIndex].intro, gap / 2, th + ch + gap / 2, gd.screenWidth - gap, ih - gap, Const.Color.WHITE);
        ...
        if (gd.shop_pageIndex == PAGE_TIP) {
            ...
            Painter.drawWordWrapString(g, gd.shop_message, 0 + gap / 2, (gd.screenHeight - ih) / 2 + gap / 2, gd.screenWidth - gap, ih - gap, Const.Color.BLACK);
        }
    }
