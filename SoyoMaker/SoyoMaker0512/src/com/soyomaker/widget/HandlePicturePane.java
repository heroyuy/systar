...
public class HandlePicturePane extends JPanel implements Scrollable,
     ...
     * @param pic
     */
    public HandlePicturePane(SpriteEditorDialog sed, Picture pic) {
        this.pic = pic;
        this.sed = sed;
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int j = 0; j < pic.getTiles().size(); j++) {
            ...
            g2d.draw3DRect((int) (tile.getSourcePoint().x * zoom), (int) (tile.getSourcePoint().y * zoom),
        ...
        if (Math.abs((int) (createTraget.getX() - createOrgine.getX())) > 0
            ...
            g2d.draw3DRect((int) Math.min(createTraget.getX(), createOrgine.getX()), (int) Math.min(createTraget.getY(), createOrgine.getY()),
            ...
            g2d.fillRect((int) Math.min(createTraget.getX(), createOrgine.getX()), (int) Math.min(createTraget.getY(), createOrgine.getY()),
        ...
        if (selectedIndex != -1) {
            ...
            if (tile != null) {
                ...
                g2d.setComposite(AlphaComposite.getInstance(
                        AlphaComposite.SRC_ATOP, 0.2f));
                g2d.fillRect((int) (tile.getSourcePoint().x * zoom), (int) (tile.getSourcePoint().y * zoom),
        ...
        if (press) {
            if (selectedIndex == -1) {
                ...
                g2d.drawString("x:" + createOrgine.getX(), createTraget.x + 1, createTraget.y + 16);
                g2d.drawString("y:" + createOrgine.getY(), createTraget.x + 1, createTraget.y + 32);
                g2d.drawString("宽:" + Math.abs(createTraget.x - createOrgine.getX()), createTraget.x + 1, createTraget.y + 48);
                g2d.drawString("高:" + Math.abs(createTraget.y - createOrgine.getY()), createTraget.x + 1, createTraget.y + 64);
            }
        }
    }
        ...
        for (int i = 0, n = pic.getTiles().size(); i < n; i++) {
            ...
            if (tile.pictureContains(cx, cy)) {
                ...
                if (e.getButton() == MouseEvent.BUTTON1) {//左键按下显示移动鼠标
                    ...
                    data.getCurProject().removeClip(selectedIndex);
                    selectedIndex = -1;
                }
                repaint();
                return;
    ...
    public void mouseMoved(MouseEvent e) {
        for (int i = 0, n = pic.getTiles().size(); i < n; i++) {
            ...
            if (tile.pictureContains(e.getX(), e.getY())) {
                this.setToolTipText("<html>切块ID:" + data.getCurProject().getClips().indexOf(tile)
                        + "<br>x:" + (tile.getSourcePoint().x)
                        + "<br>y:" + (tile.getSourcePoint().y)
                        + "<br>宽度:" + tile.getW()
                        + "<br>高度:" + tile.getH()
                        + "</html>");
            }
