...
public class DefaultMapLuaWriter implements IMapWriter {
    ...
    public void writeMap(Map map, String filename) throws Exception {
        ...
        lts.addNode("\n");
        lts.addNode("index", Configuration.Prefix.MAP_MASK + map.getIndex() + 1);
        lts.addNode("\n");
        ...
        for (int j = 0; j < map.getTileSets().size(); j++) {
            TileSet set = map.getTileSets().get(j);
            LuaTable ef = new LuaTable();
            ef.addNode("id", (AppData.getInstance().getCurProject().indexOfTileSet(set) + 1));
            boolean auto = set.isAutoTile();
            if (auto) {
                //将拼好的自动图集的image写入文件中
                File dir = new File(AppData.getInstance().getCurProject().getPath() + "/image/autotile/tileset");
                if (!dir.exists()) {
                    dir.mkdirs();
                }
                ImageIO.write(set.getAutoTileGenerateImage(), "png", new File(dir, set.getTilebmpFile().getName()));
                ef.addNode("path", "/image/autotile/tileset/" + set.getTilebmpFile().getName());
            } else {
                ef.addNode("path", "/image/tileset/" + set.getTilebmpFile().getName());
            }
            efs.addNode("[" + (AppData.getInstance().getCurProject().indexOfTileSet(set) + 1) + "]", ef);
                ...
                for (int m = 0; m < map.getHeight(); m++) {
                    ...
                    for (int k = 0; k < map.getWidth(); k++) {
                        ...
                        if (tile == null) {
                            ...
                            ls.addNode(AppData.getInstance().getCurProject().indexOfTileSet(tile.getTileSet()) + 1);
                            if (tile.getTileSet().isAutoTile()) {
                                ls.addNode(tile.getAutoId());
                            } else {
...
//                System.out.println("SpriteLayer");
                ...
                for (int j = 0; j < map.getHeight(); j++) {
                    for (int k = 0; k < map.getWidth(); k++) {
                        ...
                        if (npc != null) {
                            colhs.addNode(Configuration.Prefix.NPC_MASK + 1 + npc.getIndex());
                        }
                    }
                }
                lts.addNode("npcs", colhs);
            }
        }
        LuaNode ln = new LuaNode("Dictionary.maps[" + (Configuration.Prefix.MAP_MASK + map.getIndex() + 1) + "]", lts);
