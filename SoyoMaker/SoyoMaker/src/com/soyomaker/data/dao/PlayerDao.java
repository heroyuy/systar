...
//        System.out.println("player load");
        AppData.getInstance().getLogger().v("player load");
...
//            System.out.println("没有可加载的Player");
            AppData.getInstance().getLogger().e("没有可加载的Player");
        }
    }
...
//            System.out.println("player.gat 写入失败!");
            AppData.getInstance().getLogger().e("player.gat 写入失败!");
            return false;
        }
    }
    ...
    private boolean saveLua() {
        ...
        LuaNode ln = new LuaNode("globalDictionary.players", lts);
        try {
...
//        System.out.println("player save");
        AppData.getInstance().getLogger().v("player save");
        boolean value = false;
        if (saveBin() && saveLua()) {
            value = true;
        }
