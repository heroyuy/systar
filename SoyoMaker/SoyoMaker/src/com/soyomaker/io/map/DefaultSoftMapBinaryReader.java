...
//            System.out.println("同步通行度设置");
            Pass pic = data.getMainFrame().tscd.initCrossFile(path.substring(path.lastIndexOf(File.separatorChar) + 1),
                    new Rectangle(map.getTileWidth(), map.getTileHeight()));
            if (pic != null) {
...
//                int n = newTileset.generateGaplessArrayList().size();
                for (int k = 0; k < pic.collides.length; k++) {
                    for (int j = 0; j < pic.collides[0].length; j++) {
                        is.getTile(k + pic.collides.length * j).setCross(!pic.collides[k][j]);
                    }
                }
            }
            map.addTileset(is);
                ...
                case Layer.SPRITELAYER:
                    ...
                    for (int j = 0; j < map.getHeight(); j++) {
                        for (int k = 0; k < map.getWidth(); k++) {
                            ...
                            INpcReader npcReader = new DefaultNpcBinaryReader();
                            ...
                            AppData.getInstance().getCurProject().addNpc(npc, npc.getIndex());
