                ...
                if (layer.getNpcAt(x, y) != null) {
                    ...
                    if (layer.getNpcAt(x, y).getNpcState().
                        ...
                        switch (layer.getNpcAt(x, y).getNpcState().getFace()) {
                            ...
                            case NpcState.DOWN:
                                g2d.drawImage(layer.getNpcAt(x, y).getNpcState().getImg(), gx + 1, gy - (int) (layer.getNpcAt(x, y).getNpcState().getImg().getHeight(null) * zoom) / 4 + 1, gx + (int) (layer.getNpcAt(x, y).getNpcState().getImg().getWidth(null) * zoom) / 4 - 1, gy - 1,
                                        0, 0, layer.getNpcAt(x, y).getNpcState().getImg().getWidth(null) / 4,
                                        layer.getNpcAt(x, y).getNpcState().getImg().getHeight(null) / 4, null);
                                break;
                            case NpcState.LEFT:
                                g2d.drawImage(layer.getNpcAt(x, y).getNpcState().getImg(), gx + 1, gy - (int) (layer.getNpcAt(x, y).getNpcState().getImg().getHeight(null) * zoom) / 4 + 1, gx + (int) (layer.getNpcAt(x, y).getNpcState().getImg().getWidth(null) * zoom) / 4 - 1, gy - 1,
