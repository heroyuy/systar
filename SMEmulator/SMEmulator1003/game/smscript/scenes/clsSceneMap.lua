...
function clsSceneMap:update()
  ...
  nsx=px-math.floor(screenColNum/2)
  nsy=py-math.floor(screenRowNum/2)
  if nsx<0 then
    nsx=0
  ...
  if nsx>sx then
    --右移
    clsSceneMap:moveRight()
  elseif nsx<sx then
    --左移
     clsSceneMap:moveLeft()
  elseif nsy>sy then
    --下移
     clsSceneMap:moveDown()
  elseif nsy<sy then
    --上移
     clsSceneMap:moveUp()
  end
  ...
  for i=0,layerNum-1 do
    ...
    for j=nsx,nsx+screenColNum-1 do
      ...
      if imageSetId~=-1 then
        ...
        local imgsx=math.mod(tiledIndex,imgColNum)*cellWidth
        local imgsy=math.floor(tiledIndex/imgColNum)*cellWidth
        g_bufferPainter:drawImage(g_imageSet[imageSetId],imgsx,imgsy,cellWidth,cellHeight,(j-nsx)*cellWidth,(rowIndex-nsy)*cellHeight,0)
      end
    end
  end
end
