...
function clsSceneBattle:turnEnd()
    ...
    self:displaySlipDamageEffectTurn()
...
-- 显示持续伤害效果
function clsSceneBattle:displaySlipDamageEffectTurn()
	local text
	for _,enemy in pairs(globalGameTroop:existingMembers()) do
		if enemy:isSlipDamageTurn() then
			if enemy:hpDamage() > 0  then
				text = string.gsub(vocab.SLIP_DAMAGE_EFFECT_TURN, "*s", enemy:name(), 1)
				table.insert(self.messageWindow.message,text)
				text = vocab.ENEMY_DAMAGE
				text = string.gsub(text, "*1$s", enemy:name(), 1)
    			text = string.gsub(text, "*2$s", enemy:hpDamage(), 1)
    			table.insert(self.messageWindow.message,text)
			end
		end
	end
	for _,actor in pairs(globalGameParty:existingMembers()) do
		if actor:isSlipDamageTurn() then
			if actor:hpDamage() > 0  then
				text = string.gsub(vocab.SLIP_DAMAGE_EFFECT_TURN, "*s", actor:name(), 1)
				table.insert(self.messageWindow.message,text)
				text = vocab.ACTOR_DAMAGE
				text = string.gsub(text, "*1$s", actor:name(), 1)
    			text = string.gsub(text, "*2$s", actor:hpDamage(), 1)
    			table.insert(self.messageWindow.message,text)
			end
		end
	end
...
function clsSceneBattle:executeActionItem()
    ...
    if self.activeBattler:isActor() then
    	globalGameParty:consumeItem(item)
    	self.itemWindow:refresh()
    end
...
function clsSceneBattle:displayAddedBuffs(target, obj)
	print("显示增加状态开始")
	local text
	for _,buff in pairs(target:addedBuffs()) do
      	if target:isActor() then
      	...
      	if buff:getIndex() == 0 then                     -- 无法战斗
      		target:performCollapse()
      	end
      	text = string.gsub(vocab:buffTerm(buff:getIndex(),target:isActor(),0), "*s", target:name(), 1)
      	table.insert(self.messageWindow.message,text) 
    end
    self.messageWindow:refresh()  
    self.waitCount = 15
...
function clsSceneBattle:displayRemovedBuffs(target, obj)
	...
	print(target)
	print("xx")
	for _,buff in pairs(target:removedBuffs()) do
