...
-- 获取一般文字颜色
function clsWindowBase:disableColor()
    return self:textColor(7)
end
-- 获取危机文字颜色
...
function clsWindowBase:drawActorHp(actor,x,y)
	...
	self.contents:drawString(actor:hp().."/"..actor:maxHp(),x+64,y+18,3)
end
-- 绘制SP
...
function clsWindowBase:drawItemName(item,x,y,itemWidth)
	local enabled = globalGameParty:isItemCanUse(item)
	if item:isA() == "Item" then
		self:drawIcon(".\\game\\image\\icon\\item\\"..item:icon(),x,y,enabled)
	elseif item:isA() == "Equip" then
		self:drawIcon(".\\game\\image\\icon\\equip\\"..item:icon(),x,y,enabled)
	end
	self.contents:setTextSize(14)
	if enabled then
		self.contents:setColor(self:normalColor())
	else
		self.contents:setColor(self:disableColor())
	end
	self.contents:drawString(item:name(),28 + x,y,0)
	self.contents:drawString(":"..globalGameParty:itemNumber(item),x + itemWidth - 68,y,0)
end
-- 绘制技能名字
function clsWindowBase:drawSkillName(skill,x,y,itemWidth,actor)
	local enabled = actor:isSkillCanUse(skill)
	self:drawIcon(".\\game\\image\\icon\\skill\\"..skill:icon(),x,y,enabled)
	if enabled then
		self.contents:setColor(self:normalColor())
	else
		self.contents:setColor(self:disableColor())
	...
	self.contents:setTextSize(14)
	self.contents:drawString(skill:name(),28 + x,y,0)
	self.contents:drawString(":"..skill:spCost(),x + itemWidth - 68,y,0)
end
--  绘制行走图
