...
//        sign = null;
    ...
    public void init() {
        ...
        gd.isFail = false;
        try {
            bar = Image.createImage("/image/skin/bar.png");
...
//            battBg = Image.createImage("/image/battleback/battbg.png");
            ...
            process = Image.createImage("/image/skin/process.png");
            arr = Image.createImage("/image/skin/arr.png");
//            signcess = Image.createImage("/image/skin/signcess.png");
//            sign = Image.createImage("/image/skin/sign.png");
            ack = Image.createImage("/image/skin/attack.png");
            def = Image.createImage("/image/skin/defend.png");
            escape = Image.createImage("/image/skin/tp.png");
            magic = Image.createImage("/image/skin/skill.png");
            item = Image.createImage("/image/skin/goods.png");
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        if (gd.attackType == 0) {
            ...
            switch (gd.Select_Item_Main) {
                case BattleControl.Item_Main:
                    ...
                    g.setColor(0);
                    g.drawString("物品", gd.screenWidth / 2, 24, Graphics.HCENTER | Graphics.TOP);
                    ...
                    if (gd.player.bag.getList(Bag.ITEM).length > 0) {
                        ...
                        Painter.drawTable(g, 25, 48, 175, 24, min, 0, texts, 0, gd.Top_Good, gd.Select_Good,
...
//                        Painter.drawScrollbar(g, Painter.SCROLLBAR_VERTICAL, 200, 48, 200, 5, 5, 10);
                        Painter.drawTipString(g, gd.gameObjectManager.getItem(gd.player.bag.getList(Bag.ITEM)[gd.Select_Good]).intro, 232, 30, 210, 30, 220, 180, 20, 0);
                    } else {
                        Painter.drawString(g, "没有物品", 24, 50, 0);
        ...
        if (gd.attackType == 0) {
            ...
            switch (gd.Select_Magic_Main) {
                case BattleControl.Magic_Main:
                    ...
                    Painter.drawDialog(g, 21, 21, 198, 24, Painter.DIALOG_DEEP);
                    g.setColor(0);
                    g.drawString("技能", gd.screenWidth / 2, 24, Graphics.HCENTER | Graphics.TOP);
                    Painter.drawDialog(g, 21, 220, 198, 24, Painter.DIALOG_DEEP);
                    if (gd.player.bag.getList(Bag.SKILL).length > 0) {
                        ...
                        Painter.drawTable(g, 25, 48, 175, 24, min, 0, texts, 0, gd.Top_Magic, gd.Select_Magic,
                            Const.Anchor.LV, Painter.DIALOG_LIGHT, Painter.CELL_DEEP);
                        Painter.drawScrollbar(g, Painter.SCROLLBAR_VERTICAL, 204, 48, 220, gd.Top_Magic, 7, gd.player.bag.getList(Bag.SKILL).length);
                        Painter.drawTipString(g, gd.gameObjectManager.getSkill(gd.player.bag.getList(Bag.SKILL)[gd.Select_Magic]).intro, 232, 30, 210, 30, 220, 180, 20, 0);
                    } else {
...
//                    g.drawImage(arr, gd.enemy[gd.Select_Magic_Eny].BattX + 5, gd.enemy[gd.Select_Magic_Eny].BattY - 10, 0);// 箭头
    ...
    public void drawBg(Graphics g) {
        ...
        g.setClip(0, 0, gd.screenWidth, gd.screenHeight);
        /************************************************/
        Painter.drawString(g, gd.player.hp + "/" + gd.player.maxHp, 140, 270, 0x303030);
        Painter.drawString(g, gd.player.sp + "/" + gd.player.maxSp, 140, 290, 0x303030);
    }

    private void battleFunction(Graphics g) {
    ...
    private void enemyAction(int id, Graphics g) {
    }
    //轮到英雄时绘制

    private void heroAction(Graphics g) {
//        System.out.println("isRuning "+gd.isRuning);
    ...
    private void moveBar(Graphics g) {
        ...
        switch (curState) {
            ...
            case MAIN: {
                ...
                for (int i = 0; i < gd.enemy.length; i++) {
                    ...
                    if (!gd.enemy[i].isDead()) {
                        g.drawString(gd.enemy[i].hp + "/" + gd.enemy[i].maxHp, gd.enemy[i].BattX, gd.enemy[i].BattY - 5, Graphics.BOTTOM | Graphics.LEFT);//显示怪物剩余血量
            ...
            case END:
                ...
                switch (gd.winSelect) {
                    case 0:
                        ...
                        Painter.drawString(g, "获得经验 " + gd.allExp, 70, 140, 0);
                        Painter.drawString(g, "获得金币 " + gd.allMoney, 70, 180, 0);
                        break;
                    case 1:
                        Painter.drawDialog(g, 40, 100, 160, 120, Painter.DIALOG_DEEP);
                        if (enemyTroop.itemList.length > 0) {
                            for (int i = 0; i < enemyTroop.itemList.length; i++) {
                                Painter.drawString(g, "获得物品 " + gd.gameObjectManager.getItem(enemyTroop.itemList[i]).name + " x1", 45, 120 + i * 20, 0);
                            }
                        } else {
                            Painter.drawString(g, "没有物品", 24, 50, 0);
                        }
