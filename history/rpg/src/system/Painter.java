...
public class Painter {
    ...
    public static void drawString(Graphics g, String str, int x, int y, int color) {
        g.setColor(color);
        g.drawString(str, x, y, Graphics.TOP | Graphics.LEFT);
    }

    public static void fillRect(Graphics g, int x, int y, int width, int height, int color) {
        g.setColor(color);
    ...
    public static void drawRoundRect(Graphics g, int x, int y, int width, int height, int anchor, int color) {
        g.setColor(color);
        g.drawRoundRect(x, y, width, height, anchor, anchor);
    }

    public static void fillRoundRect(Graphics g, int x, int y, int width, int height, int anchor, int color) {
        g.setColor(color);
        g.fillRoundRect(x, y, width, height, anchor, anchor);
    }

    public static void drawTriangle(Graphics g, int x1, int y1, int x2, int y2, int x3, int y3, int color) {
        g.setColor(color);
     ...
     */
    public static void drawDialog(Graphics g, int x, int y, int w, int h, byte type) {
        if (type == Painter.DIALOG_DEEP || type == Painter.DIALOG_LIGHT) {
            Image img = ImageManager.getInstance().getImage(type == Painter.DIALOG_DEEP ? Const.ImagePath.DIALOG_DEEP : Const.ImagePath.DIALOG_LIGHT);
     ...
     */
    public static void drawScrollbar(Graphics g, byte style, int a, int b1, int b2, int topIndex, int showNum, int maxNum) {
        ...
        switch (style) {
            case Painter.SCROLLBAR_VERTICAL: {
                ...
                for (int i = 0; i < num; i++) {
                    g.drawImage(trench, a + wid / 2, b1 + i * trench.getHeight(), Const.Anchor.HT);
                }
    ...
    public static Image createAlphaImage(Image img, int alpha) {
        int w = img.getWidth();
        int h = img.getHeight();
        int[] imgData = new int[w * h];
        img.getRGB(imgData, 0, w, 0, 0, w, h);
        for (int i = 0; i < imgData.length; i++) {
            if ((imgData[i] & alpha) == alpha) {
                imgData[i] = imgData[i] & alpha;
            }
        }
        img = Image.createRGBImage(imgData, w, h, true);
        return img;
    }

//    public static void drawTab(Graphics g, int x, int y, int w, int h, int curIndex, String[] title) {
//    }
    /**
     * 在指定位置绘制指定大小的半透明区域
     * @param g 画笔
     ...
     * @param alpha Alpha值
     */
    public static void drawAlphaRect(Graphics g, int x, int y, int wid, int hei, int[] imgData, int color, byte alpha) {

        if (imgData == null) {
            imgData = new int[wid * hei];

            for (int i = 0; i < imgData.length; i++) {
                imgData[i] = color;
                byte a[] = new byte[4];
                a[0] = (byte) ((imgData[i] >> 24) & 0xff);
                a[1] = (byte) ((imgData[i] >> 16) & 0xff);
                a[2] = (byte) ((imgData[i] >> 8) & 0xff);
                a[3] = (byte) ((imgData[i]) & 0xff);

                a[0] = alpha;

                imgData[i] = ((a[0] & 0xff) << 24) | ((a[1] & 0xff) << 16) | ((a[2] & 0xff) << 8) | ((a[3] & 0xff));
            }
        }
        g.drawRGB(imgData, 0, wid, x, y, wid, hei, true);
    }
     ...
     */
    public static void drawCell(Graphics g, int x, int y, int w, int h, byte type) {
        Image img = ImageManager.getInstance().getImage(type == Painter.CELL_DEEP ? Const.ImagePath.CELL_DEEP : Const.ImagePath.CELL_LIGHT);
        drawRectFrame(g, img, x, y, w, h);
    }

    public static void drawStringOnCell(Graphics g, String text, int color, int x, int y, int w, int h, int anchor, byte type) {
        ...
        switch (anchor) {
            ...
            case Const.Anchor.LV:
                g.drawString(text, x + gap, y + (h - g.getFont().getHeight()) / 2, Const.Anchor.LT);
                break;
            ...
            case Const.Anchor.HV:
                g.drawString(text, x + w / 2, y + (h - g.getFont().getHeight()) / 2, Const.Anchor.HT);
                break;
            ...
            case Const.Anchor.RV:
                g.drawString(text, x + w - gap, y + (h - g.getFont().getHeight()) / 2, Const.Anchor.RT);
                break;
     ...
     */
    public static void drawTable(Graphics g, int x, int y, int cellWidth, int cellHeight, int cellNum, int gap, String[] texts, int color, int topIndex, int curIndex, int anchor, byte style, byte type) {
    ...
    public static void drawTab(Graphics g, int x, int y, int w, int h, byte type, int curIndex, String[] titles) {
        //绘制所有耳朵
        int wid = 48, hei = 24;
        ...
        for (int i = 0; i < titles.length; i++) {
            g.drawString(titles[i], x + i * wid + wid / 2, y + (hei - g.getFont().getHeight()) / 2, Const.Anchor.HT);
        }
    }

    public static void drawRectFrame(Graphics g, Image img, int x, int y, int w, int h) {
        g.setClip(x, y, w, h);
...
//    private static final int FONT_HEIGHT = 11;      //字体高度 font.getHeight() 不准确
     ...
     */
    public static void drawTipString(Graphics g, String str, int height, int TIPSTR_LEFT, int TIPSTR_RIGHT, int rectX, int rectY, int rectWidth, int rectHeight, int color) {
        g.setColor(color);
        int strWidth = g.getFont().stringWidth(str);
        int strHeight = g.getFont().getHeight();
        if (strWidth < rectWidth) {
            g.drawString(str, rectX, height - strHeight / 2, Graphics.LEFT | Graphics.TOP);
            return;
        }
        ...
        g.drawString(str, tipStringPos, height - strHeight / 2, Graphics.LEFT | Graphics.TOP);
        g.setClip(oldClipX, oldClipY, oldClipWidth, oldClipHeight);

    }

    public static void drawImage(Image src, int x_src, int y_src, int width, int height, int x_dest, int y_dest, int anchor, Graphics g) {
     ...
     */
    public static void drawNumber(Graphics g, int number, int x, int y, int w, int type, int atype) {
        ImageManager im = ImageManager.getInstance();
...
//        System.gc();
    ...
    public static void drawStringShadow(Graphics g, int mainColor, int topColor, int bottomColor, String str, int x, int y, int anchor) {
        if (topColor >= 0) {
            g.setColor(topColor);
     ...
     * @param color 文本颜色
     */
    public static void drawWordWrapString(Graphics g, String str, int x, int y, int wid, int hei, int color) {
        g.setClip(x, y, wid, hei);
        g.setColor(color);
        char[] txt = str.toCharArray();
        for (int i = 0, col = 0, line = 0; i < txt.length; i++) {
            g.drawChar(txt[i], x + col * (g.getFont().charWidth(txt[0]) + 1), y + line * (g.getFont().getHeight()), Graphics.TOP | Graphics.LEFT);
            if ((col + 1) * (g.getFont().charWidth(txt[0]) + 1) >= wid) {
                col = 0;
                line++;
            } else {
                col++;
