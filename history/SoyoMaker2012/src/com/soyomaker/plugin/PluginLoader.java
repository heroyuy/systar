...
public class PluginLoader extends URLClassLoader {
    ...
    private JButton getButton(int id) {
        ...
        try {
            ...
            Logger.getLogger(this.getClass().getName()).error("getButton " + id, ex);
//            Log.getLogger(PluginLoader.class).error("readPlugin:actionPerformed", ex);
    ...
    private JMenu getMenu(int id) {
        ...
        try {
            ...
            Logger.getLogger(this.getClass().getName()).error("getMenu " + id, ex);
//            Log.getLogger(PluginLoader.class).error("readPlugin:actionPerformed", ex);
        }
        return item;
    }
    ...
    private JMenuItem getMenuItem(int id) {
        ...
        try {
            ...
            Logger.getLogger(this.getClass().getName()).error("getMenuItem " + id, ex);
//            Log.getLogger(PluginLoader.class).error("readPlugin:actionPerformed", ex);
        }
        return item;
    }
     ...
     * @return
     */
    public boolean readPlugin(ILoggerUI iLogger) {
...
//                System.out.println(attr.getValue("Plugin-Author"));
            ...
            if (attr.getValue("Plugin-ExpandPoint") != null) {
                plugin.setExpandPoint(Integer.parseInt(attr.getValue("Plugin-ExpandPoint")));
            }
            if (attr.getValue("Plugin-Main") != null) {
                try {
                    plugin.setMain(findClass(attr.getValue("Plugin-Main")));
                } catch (ClassNotFoundException ex) {
                    Logger.getLogger(this.getClass().getName()).error("getClass " + jar, ex);
                }
            }
            if (plugin.getExpandPoint() != -1) {//插件类型为必要标示
                return plugin;
            }
        } else {
