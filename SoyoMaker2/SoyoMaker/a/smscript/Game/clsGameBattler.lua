...
function clsGameBattler:isBuffIgnore(buffIndex)
	for _,buff in pairs(self:buffs()) do
		if buff.buffs[buffIndex] ~= nil and not globalDataManager:getModel(const.DATA_MANAGER_BUFF,buffIndex).buffs[buff.index] ~= nil then
			if buff.buffs[buffIndex] < 0 then 
				return true
			end
		end
	end
...
function clsGameBattler:isBuffOffset(buffIndex)
	...
	for _,i in pairs(self.reader.buffs) do
		if globalDataManager:getModel(const.DATA_MANAGER_BUFF,buffIndex).buffs[i] ~= nil and globalDataManager:getModel(const.DATA_MANAGER_BUFF,buffIndex).buffs[i] < 0 then
			return true
		end
	end
	return false
...
function clsGameBattler:addBuff(buffIndex)
	...
	if not self:isBuff(buffIndex) then                                                      -- ״̬�Ƿ����
		...
		for i,v in pairs({}) do                                                     -- ��ȡ״̬�����״̬�б�
			if v > 0 and smRandom:nextInt(100) < math.abs(v) then                           -- Ϊ����״̬
				if not self:isBuffResist(i,v) then                                            -- �ж�״̬�Ƿ���Ч
					if not self:isDead() then                                               -- �ж��Ƿ��޷�ս��
						if not (i == 0 and self.immortal) then                -- �ж��Ƿ�Ϊ������
							if self:isBuff(i) then
								table.insert(self.reader.remainedBuffs,i)
							else
								if smRandom:nextInt(100) < self:buffProbability(i,v) then   -- ����״̬����
									self:addBuff(i)                                         -- ����״̬
									table.insert(self.reader.addedBuffs,i)                  -- ��¼�Ѹ���״̬
								end
							end
						end
					end
			...
			if v < 0 then                                                                   -- Ϊ���״̬
				if smRandom:nextInt(100) < math.abs(v) then
					self:removeBuff(i)
	...
	if buff.lastTime ~= nil then
    	if os.time() - self.buffLast[buff.index] > buff.lastTime then
    		if smRandom:nextInt(100) < buff.autoReleaseProb then
    			self:removeBuff(buff.index)
      			table.insert(self.reader.removedBuffs,buff.index)
      			return true
    		end
    	end
    end
	...
	if getmetatable(getmetatable(obj)) == clsGameBattler then
    	...
    	for i = 0,obj:plusSize()-1 do 
    			...
    			if not self:isDead() then                                              -- �ж��Ƿ��޷�ս��
    				if not (obj:plusBuff(i):getIndex() == 0 and self.immortal) then               -- �ж��Ƿ�Ϊ������
    					if self:isBuff(obj:plusBuff(i):getIndex()) then
    						...
    						print(self:buffProbability(obj:plusBuff(i):getIndex(),obj:plusBuff(i):value()))
    		...
    		for i = 0,obj:minusSize()-1 do                                                 -- ״̬�仯(-)
    			if self:isBuff(obj:plusBuff(i):getIndex()) then                                                 -- �ж�״̬�Ƿ��Ѵ���
